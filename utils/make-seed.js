    ...
    if (projectCount) {
        for (let p = 1; p <= projectCount; p++) {
            let project = {
                id: uuid.v4(),
                account_id: a,
                user_id: accountUsers[a][Math.floor(Math.random() * accountUsers[a].length)],
    ...
    if (assetCount) {
        for (let ac = 1; ac <= assetCount; ac++) {
            let asset = {
                ...
                user_id: accountUsers[projects[p].account_id][Math.floor(Math.random() * accountUsers[projects[p].account_id].length)],
